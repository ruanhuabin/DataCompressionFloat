...
{
      printf("\nUsage:\n\n");
      printf("\t%s -i <input file> -o <output file> [-b <bits to erase>]", argv[0]);
      printf("\nwhere:\n");
      printf("\t");
      printf("-i\tinput file that need to erase lowest byte\n\n");
      printf("\t");
      printf("-o\t output file that save the float number with lowerest byte to be \\0\n\n");
      printf("\t");
...
{ 
    ...
    if( inputFile == NULL )
    {
        fprintf(stderr, "[%s:%d] open file [%s] failed\n", __FILE__, __LINE__, inputFileName);
        exit(-1);
    }
 
    if( outputFile == NULL )
    {
        fprintf(stderr, "[%s:%d] open file [%s] failed\n", __FILE__, __LINE__, outputFileName);
        exit(-1);
    }
