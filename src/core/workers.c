...
void init_zips_ex(mzip_t zips[], int chk)
{
     ...
     *  Initial 4 zips to compress 4 byte stream in float number sequence
     */
    mzip_init_ex(&zips[0], chk, ZLIB_DEF, ZIP_FAST);
    mzip_init_ex(&zips[1], chk, ZLIB_DEF, ZIP_FAST);
    mzip_init_ex(&zips[2], chk, ZLIB_DEF, ZIP_FAST);
    mzip_init_ex(&zips[3], chk, ZLIB_DEF, ZIP_FAST);
}
...
int runDecompressionEx(FILE *fin, ctx_t *ctx, nz_header *hd, FILE *fout)
{
    uint32_t i, j, num;
    uint32_t chk;
    uint64_t fsz;
    ...
    chk = fsz % chk;
    printf("chk = %d\n", chk);
    printf("start to process remain\n");
    if(chk > 0)
    {
        _unzip_1_Ex(fin, zips, outs, chk);
...
int runCompressEx(FILE *fin, ctx_t *ctx, FILE *fout, const int bitsToMask)
{
    ...
    while(num > 0)
    {
        ...
        flushFlag = feof(fin) ? Z_FINISH:Z_NO_FLUSH;
        begin = now_sec();
        for(j = 0; j < 4; j ++)
        {
            zbegin        = now_sec();
             ...
             *    the length of each compressed byte stream is stored in zlens[0..3]
             */
            zips[j].zipfun_ex(&(zips[j]), &zouts[j], &zlens[j], flushFlag, fout);

            /*
             *mzlib_def_ex(&zips[j], &zouts[j], &zlens[j], flushFlag, fout);
             */
            zips[j].time1 += (now_sec() - zbegin);
        }
        ctx->zipTime += (now_sec() - begin);
